                            ...
                            <div class="portlet light bordered">
                                <div class="portlet-title">
                                    ...
                                    <div class="btn btn-primary font-dark pull-right">
                                        <a href="{{url("admin/news/add")}}" style="color: white"> <i class="glyphicon glyphicon-plus"></i>
                                            新增</a>
                                    </div>
                                </div>
                                <form class="form-horizontal" role="form" action="{{url("admin/news")}}" method="post">
                                    {!! csrf_field() !!}
                                    <div class="row">
                        ...
                        <div class="tab-pane active" id="portlet_comments_1">
                            ...
                            <div class="portlet light bordered">
                                <div class="portlet-title">
                                    <div class="caption font-dark">
                                        <i class="icon-settings font-dark"></i>
                                        <span class="caption-subject bold uppercase"> 活动公告管理</span>
                                    </div>
                                    ...
                                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                                        ...
                                        @foreach($newsList as $key=>$news)
                                            <tr class="odd gradeX">
                                                <td> {{$news->id}}</td>
                                                <td> {{$news->title}}</td>
                                                <td> {{$news->hasOneMenu["name"]}}</td>
                                                <td> {{$news->created_at}}</td>
                                                <td>
                                                    {{--<a href="" class="btn btn-default" data-toggle="modal" data-target="#myModal">查看</a>--}}
                                                    <a href="{{url("admin/news/edit/$news->id")}}" class="btn btn-default">修改</a>
                                                    <a href="{{url("admin/news/del/$news->id")}}" class="btn btn-danger">删除</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
                                    </table>
                                    {!! $newsList->appends($cdatas)->render() !!}
                                </div>
                            </div>
                            <!-- END: Comments -->
                        </div>
...
@section('script')
    <script>
        ...
        fillSelectServer("news","mode",defaultVal,true);
    </script>
    @endsection
