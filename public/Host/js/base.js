...
$(function(){
		...
		$(window).scroll(function(){
			if($(window).scrollTop() >= 80){
 		      ...
 		      $(".child-box").css('top',60);
 			}else{
				$("#header").removeClass("head-pinned");
				$(".child-box").css('top',80);
			}
		});
...
$(function(){
 	var time = null;
		...
		var t=$(this),
		    index   = t.index(),
			ch      = t.find(".child-box"),
			chi= t.find(".child-box2"),
			cha= t.find(".child-box2 .sub-menu");
 			
			
 		    t.find('a:first').stop().addClass("now");
 			if(t.find('ol').hasClass("sub-menu")){
			chi.find('.sub-menut dt').find('a:first').addClass('opcat');	
  		   chi.find('.sub-menut ol.sub-menu dd a').addClass('opcats');
   				}
      		if(t.find("div").hasClass("child-box")){
 		  ch.hide().show();
  		 var _height =chi.height(),
             _w=0,
 		     _left=t.offset();
 		 _left=parseInt(_left.left+t.width()/2);
		 chi.find("dt").each(function(){_w += $(this).width()+20; });
  		 chi.find("dt:first").css("padding-left",_left-_w/2);	
  		 ch.stop().animate({height:_height,opacity:1},400);
  		}
  	},function(){
 	    $(this).find(".child-box").stop().animate({height:0,opacity:0},400).hide();
...
$("#nav").toggle(100,function(){
	if($(this).is(":hidden"))
	{
	$(".navbox a").removeClass("on");	
 		}
	else{
		$(".navbox a").addClass("on");
		}	
 	});	
	});
